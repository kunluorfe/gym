...
class OUTradingEnv(gym.Env):
    def __init__(self):
        ...
        self.sigma = .4
        self.gamma = 2
        self.lmda = 1  ## mean reverting speed
        self.r = .05
        self.N = 100  ## steps???
        self.tc = 0.004 ## linear transaction cost
        self.Xinit = 1
        self.Winit = 10
        self.Xlow = 0.1
        self.Wlow = 1
        ...
        self.action_space = spaces.Discrete(self.num_actions * 2 + 1) # 0 to 100
    ...
    def util(self,w):
        if self.gamma == 0:
            ...
            return (np.power(w,1-self.gamma)-1) / (1-self.gamma)
    ...
    def _step(self, action):
        ...
        at = (action - self.num_actions ) / self.num_actions * W / X  ## actions ranges from -1:0.02:1??
        dW = at * (self.mu - X) * (1 - exp(-self.lmda * dt)) + (W - at * X) * (exp(self.r * dt) - 1) + at * Ntdelta
        C = self.tc * abs(prev-at) * X
        X = X + dX
        dJ = util(W+dW-C)-util(W)
        W = W + dW - C
        tau = tau - dt
        prev_shares = at
